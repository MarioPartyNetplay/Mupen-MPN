 ...
 * - Level 1: Light throttling (75% speed) - when buffer > target + 1
 * - Level 2: Medium throttling (50% speed) - when buffer > target + 2 with sustained lag
 * - Level 3: Heavy throttling (25% speed) - when buffer > target + 3 with sustained lag
 * 
 * Throttling is applied when:
 * 1. Buffer health exceeds target by more than 1
 * 2. Player has sustained lag for more than 3 seconds
 * 3. Buffer health is critically high (> target + 8)
 * 
...
static uint8_t calculate_throttle_level(uint8_t control_id)
{
    ...
    if (buffer_health > l_buffer_target + 1)
    {
        throttle_level = 1;
...
static uint32_t netplay_get_input(uint8_t control_id)
{
    ...
    if (l_player_throttle_level[control_id] > 0)
    {
        ...
        {
            case 1:
                speed_factor = 75;  // 75% speed
                break;
            case 2:
                speed_factor = 50;  // 50% speed
                break;
            case 3:
                speed_factor = 25;  // 25% speed
                break;
            default:
...
void netplay_get_throttling_stats(uint8_t* throttle_levels, uint8_t* buffer_health, uint8_t* player_lags)
{
    ...
    for (int i = 0; i < 4; ++i)
    {
        throttle_levels[i] = l_player_throttle_level[i];
        buffer_health[i] = buffer_size(i);
        player_lags[i] = l_player_lag[i];
    }
}
