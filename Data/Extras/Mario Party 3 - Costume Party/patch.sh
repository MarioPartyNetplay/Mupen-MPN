#!/bin/bash

# Get a list of ISO files in the same directory as the script
iso_files=$(find "$(dirname "$0")" -maxdepth 1 -type f -iname "*.z64")

# Check if there are no ISO files
if [ -z "$iso_files" ]; then
  echo ""
  echo "No Z64 files found in the same directory as the script."
  echo "Place the Mario Party 3 Z64 in the script's directory and run it again."
  echo ""
  read -p "Press Enter to exit..."
  exit 0
fi

# Iterate over each ISO file and process it
for originalISO in $iso_files; do
  echo ""
  echo "Given \"$originalISO\""
  xdelta3 -d -s "$originalISO" data.xdelta3 "Mario Party 3 [Costume Party].iso"
  echo "Press Enter to exit."
  read -p ""
done

echo ""
read -p "Press Enter to exit..."
exit 0